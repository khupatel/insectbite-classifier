...
ddi_model = Model(inputs=base_model.input, outputs=ddi_output)

# optimizer
optimizer = Adam(learning_rate=1e-4)

ddi_model.compile(optimizer=optimizer,
...
base_model_B = DenseNet169(weights='imagenet', include_top=False, input_shape=(img_height, img_width, 3))
...
# final model
insect_model = models.Model(inputs=ddi_model.input, outputs=x)
...
history = insect_model.fit(train_ds,
                    validation_data=val_ds,
                    epochs=30,
                    batch_size=batch_size)

#---------------
# Add weights of A and B
...
for layer in ddi_model.layers[-20:]:
