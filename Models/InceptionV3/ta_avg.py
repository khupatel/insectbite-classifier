...
for layer_insect, layer_ddi in zip(insect_model.layers[:-1], ddi_model.layers[:-1]):
    new_weights = [(w_insect + w_ddi) / 2 for w_insect, w_ddi in zip(layer_insect.get_weights(), layer_ddi.get_weights())]
...
history_insect_finetune = insect_model.fit(train_ds, validation_data=val_ds, epochs=50)
